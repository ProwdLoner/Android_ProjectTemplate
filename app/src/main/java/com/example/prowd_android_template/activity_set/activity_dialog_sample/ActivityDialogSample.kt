...
class ActivityDialogSample : AppCompatActivity() {
    ...
    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityDialogSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityDialogSampleSpw
    ...
    private fun onCreateInitObject() {
        ...
        adapterSetMbr = ActivityDialogSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityDialogSampleSpw(application)
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityDialogSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityDialogSampleSpw",
...
//            }
    ...
    class ActivityDialogSampleAdapterSet {
        // 어뎁터 #1
