package com.example.prowd_android_template.activity_set.activity_view_pager_sample_list
...
class ActivityViewPagerSampleList : AppCompatActivity() {
    ...
    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityViewPagerSampleListAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityViewPagerSampleListSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityViewPagerSampleListBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityViewPagerSampleListAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityViewPagerSampleListSpw(application)
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityViewPagerSampleListSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityViewPagerSampleListSpw",
...
//            }
    ...
    class ActivityViewPagerSampleListAdapterSet {
        // 어뎁터 #1
