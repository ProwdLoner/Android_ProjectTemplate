package com.example.prowd_android_template.activity_set.activity_view_to_bitmap_sample
...
import com.example.prowd_android_template.databinding.ActivityViewToBitmapSampleBinding
...
class ActivityViewToBitmapSample : AppCompatActivity() {
    // <설정 변수 공간>
    // (앱 진입 필수 권한 배열)
    // : 앱 진입에 필요한 권한 배열.
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityViewToBitmapSampleBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityViewToBitmapSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityViewToBitmapSampleSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityViewToBitmapSampleBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityViewToBitmapSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityViewToBitmapSampleSpw(application)
    ...
    private fun onCreateInitView() {
        bindingMbr.captureBtn.setOnClickListener {
            // 뷰를 비트맵으로
            val screenBitmap =
                CustomUtil.getBitmapFromView(bindingMbr.capturedView)
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityViewToBitmapSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityViewToBitmapSampleSpw",
...
//            }
    ...
    class ActivityViewToBitmapSampleAdapterSet {
        // 어뎁터 #1
