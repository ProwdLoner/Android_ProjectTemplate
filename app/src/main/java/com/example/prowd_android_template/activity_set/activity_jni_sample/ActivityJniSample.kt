...
class ActivityJniSample : AppCompatActivity() {
    ...
    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityJniSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityJniSampleSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityJniSampleBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityJniSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityJniSampleSpw(application)
    ...
    private fun onCreateInitView() {
        ...
        bindingMbr.helloFromTestBtn.setOnClickListener {
            bindingMbr.helloFromTestResultValue.text = NativeWrapperJniStringTest.getJniString()
        }
    }
...
//            }
    ...
    class ActivityJniSampleAdapterSet {
        // 어뎁터 #1
