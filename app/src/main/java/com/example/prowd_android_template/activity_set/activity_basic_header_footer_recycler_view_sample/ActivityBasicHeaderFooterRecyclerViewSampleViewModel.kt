...
class ActivityBasicHeaderFooterRecyclerViewSampleViewModel(application: Application) :
    ...
    fun getRecyclerViewHeaderData(
        executorOnComplete: (GetRecyclerViewHeaderDataOutputVO) -> Unit,
        executorOnError: (Throwable) -> Unit
    ) {
        executorServiceMbr?.execute {
            Thread.sleep(1000)

            executorOnComplete(GetRecyclerViewHeaderDataOutputVO("Header Test"))
        }
    }

    // 푸터 데이터 요청
    fun getRecyclerViewFooterData(
        executorOnComplete: (GetRecyclerViewFooterDataOutputVO) -> Unit,
        executorOnError: (Throwable) -> Unit
    ) {
        executorServiceMbr?.execute {
            Thread.sleep(1000)

            executorOnComplete(GetRecyclerViewFooterDataOutputVO("Footer Test"))
        }
    }
