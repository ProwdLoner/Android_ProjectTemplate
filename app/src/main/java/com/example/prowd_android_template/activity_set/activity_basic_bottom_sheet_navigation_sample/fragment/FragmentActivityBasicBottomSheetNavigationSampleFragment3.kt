...
class FragmentActivityBasicBottomSheetNavigationSampleFragment3 : Fragment() {
    ...
    override fun onResume() {
        ...
        if (!parentViewModel.isChangingConfigurationsMbr && // 화면 회전이 아니면서,
            ...
            if (parentViewModel.isFragment3DataFirstLoadingMbr || // 데이터 최초 로딩 시점일 때 혹은,
                sessionToken != parentViewModel.fragment3CurrentUserSessionTokenMbr // 액티비티 유저와 세션 유저가 다를 때
            ) {
                // 진입 플래그 변경
                parentViewModel.isFragment3DataFirstLoadingMbr = false
                parentViewModel.fragment3CurrentUserSessionTokenMbr = sessionToken
    ...
    private fun createViewModelDataObjects() {
        if (!parentViewModel.isChangingConfigurationsMbr) { // 설정 변경(화면회전)이 아닐 때에 발동
            ...
            // 현 액티비티 진입 유저 저장
            parentViewModel.fragment3CurrentUserSessionTokenMbr =loginInfo.sessionToken
        }
    }
