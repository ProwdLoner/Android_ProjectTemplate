...
class ActivityBasicHorizontalRecyclerViewSample : AppCompatActivity() {
    ...
    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityBasicHorizontalRecyclerViewSampleSpw
    ...
    // <공개 메소드 공간>
    // (아이템 내용 변경)
    fun putActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterData(
        serverUid: Long,
        text: String,
        onComplete: () -> Unit
    ) {
                        ...
                        1 -> {// 완료
                            ...
                            val idx =
                                cloneItemList.indexOfFirst {
                                    (it as ActivityBasicHorizontalRecyclerViewSampleAdapterSet.RecyclerViewAdapter.Item1.ItemVO)
                                        .serverItemUid == serverUid
                                }

                            (cloneItemList[idx] as ActivityBasicHorizontalRecyclerViewSampleAdapterSet.RecyclerViewAdapter.Item1.ItemVO).title =
                                text
    ...
    // (아이템 삭제)
    fun deleteActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterData(
        serverUid: Long,
        onComplete: () -> Unit
    ) {
        executorServiceMbr.execute {
    ...
    private fun onCreateInitObject() {
        ...
        adapterSetMbr = ActivityBasicHorizontalRecyclerViewSampleAdapterSet(
            ActivityBasicHorizontalRecyclerViewSampleAdapterSet.RecyclerViewAdapter(
                ...
                onScrollReachTheEnd = {
                    getActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterNextPageDataList(
                        onComplete = {}
                    )
                }
            )
        ...
        // SPW 객체 생성
        classSpwMbr = ActivityBasicHorizontalRecyclerViewSampleSpw(application)
    ...
    private fun onCreateInitView() {
        ...
        bindingMbr.addItemBtn.setOnClickListener {
            postActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterData(
                "추가 아이템",
                onComplete = {})
        }
    ...
    // ActivityBasicHorizontalRecyclerViewSampleAdapterSet - RecyclerViewAdapter 의 다음 페이지 데이터 리스트 가져오기
    // : serverItemUid 를 네트워크 요청에 입력하면 서버에선 해당 uid 이후의 데이터 리스트를 반환
    private var getActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterNextPageDataListOnProgressMbr =
        false

    private fun getActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterNextPageDataList(
        onComplete: () -> Unit
    ) {
        if (getActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterNextPageDataListOnProgressMbr) {
            return
        }
        getActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterNextPageDataListOnProgressMbr =
            true
            ...
            executorServiceMbr.execute {
                ...
                val lastItemServerUid = if (cloneItemList1.size < 1) { // 아이템 리스트에 실질적인 아이템이 없을 때,
                    ...
                    (cloneItemList1[cloneItemList1.lastIndex - 1] as
                            ActivityBasicHorizontalRecyclerViewSampleAdapterSet.RecyclerViewAdapter.Item1.ItemVO).serverItemUid
                }
    ...
    // (아이템 추가)
    private fun postActivityBasicHorizontalRecyclerViewSampleAdapterSetRecyclerViewAdapterData(
        text: String,
        onComplete: () -> Unit
    ) {
        executorServiceMbr.execute {
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityBasicHorizontalRecyclerViewSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityBasicHorizontalRecyclerViewSampleSpw",
