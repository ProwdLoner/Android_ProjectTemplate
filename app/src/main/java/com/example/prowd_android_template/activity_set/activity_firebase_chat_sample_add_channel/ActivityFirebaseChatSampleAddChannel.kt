package com.example.prowd_android_template.activity_set.activity_firebase_chat_sample_add_channel
...
import com.example.prowd_android_template.databinding.ActivityFirebaseChatSampleAddChannelBinding
import com.example.prowd_android_template.databinding.ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItem1Binding
import com.example.prowd_android_template.databinding.ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItemLoaderBinding
...
// [멀티 채팅을 위한 채팅방 생성 샘플]
// 아래 유저 리스트는 ActivityFirebaseChatSampleAddChannel 의 목록을 사용하는데, 실제로 채팅방을 만드는 상황을 고려하면,
// 친구 목록에서 추가, 생성된 채팅방 내에서 추가, 검색하여 추가
// 이렇게 기능을 구현하여 사용하면 됩니다.
class ActivityFirebaseChatSampleAddChannel : AppCompatActivity() {
    // <설정 변수 공간>
    // (앱 진입 필수 권한 배열)
    // : 앱 진입에 필요한 권한 배열.
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityFirebaseChatSampleAddChannelBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityFirebaseChatSampleAddChannelAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityFirebaseChatSampleAddChannelSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityFirebaseChatSampleAddChannelBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityFirebaseChatSampleAddChannelAdapterSet(
            ActivityFirebaseChatSampleAddChannelAdapterSet.RecyclerViewAdapter(
                this,
        ...
        // SPW 객체 생성
        classSpwMbr = ActivityFirebaseChatSampleAddChannelSpw(application)
    ...
    private fun onCreateInitView() {
        if (adapterSetMbr.recyclerViewAdapter.userCheckedItemUidListMbr.size == 0) {
            bindingMbr.createChannelContainer.setBackgroundColor(
                Color.parseColor("#FF999999")
            )
            bindingMbr.createChannelContainer.isEnabled = false
        } else {
            bindingMbr.createChannelContainer.setBackgroundColor(
                Color.parseColor("#FF3D7DDC")
            )
            bindingMbr.createChannelContainer.isEnabled = true
        }

        bindingMbr.createChannelContainer.setOnClickListener {
            // todo
        ...
        executorServiceMbr.execute {
            ...
            runOnUiThread {
                ...
                adapterSetMbr.recyclerViewAdapter.setItemList(
                    arrayListOf(
                        ActivityFirebaseChatSampleAddChannelAdapterSet.RecyclerViewAdapter.ItemLoader.ItemVO(
            ...
            for (sampleUser in ActivityFirebaseChatSampleLogin.sampleUserList) {

                // 자기 자신은 리스트에서 빼기
                if (intent.getLongExtra("userUid", -1) == sampleUser.userUid) {
                    continue
                }

                resultObj.add(
                    ActivityFirebaseChatSampleAddChannelAdapterSet.RecyclerViewAdapter.Item1.ItemVO(
                        adapterSetMbr.recyclerViewAdapter.nextItemUidMbr,
                        sampleUser.userUid,
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityFirebaseChatSampleAddChannelSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityFirebaseChatSampleAddChannelSpw",
...
//            }
    ...
    class ActivityFirebaseChatSampleAddChannelAdapterSet(
        val recyclerViewAdapter: RecyclerViewAdapter
    ) {
        // 어뎁터 #1
        class RecyclerViewAdapter(
            private val parentViewMbr: ActivityFirebaseChatSampleAddChannel,
            ...
            // <멤버 변수 공간>
            // checkbox check 처리된 유저
            val userCheckedItemUidListMbr = ArrayList<Long>()
                ...
                return when (viewType) {
                    ...
                    ItemLoader::class.hashCode() -> {
                        ItemLoader.ViewHolder(
                            LayoutInflater.from(parent.context)
                                .inflate(
                                    R.layout.item_activity_firebase_chat_sample_add_channel_adapter_recycler_view_item_loader,
                                    parent,
                                    false
                                )
                        )
                    ...
                    Item1::class.hashCode() -> {
                        Item1.ViewHolder(
                            LayoutInflater.from(parent.context)
                                .inflate(
                                    R.layout.item_activity_firebase_chat_sample_add_channel_adapter_recycler_view_item1,
                                    parent,
                                    false
                                )
                        )
...
//                    val copyEntity = currentDataListCloneMbr[position] as ItemLoader.ItemVO
                    ...
                    is Item1.ViewHolder -> { // 아이템1 아이템 바인딩
                        ...
                        binding.userCheck.isChecked =
                            userCheckedItemUidListMbr.contains(copyEntity.itemUid)

                        binding.userCheck.setOnClickListener {
                            if (binding.userCheck.isChecked) {
                                userCheckedItemUidListMbr.add(copyEntity.itemUid)

                                if (userCheckedItemUidListMbr.size > 0) {
                                    parentViewMbr.bindingMbr.createChannelContainer.setBackgroundColor(
                                        Color.parseColor("#FF3D7DDC")
                                    )
                                    parentViewMbr.bindingMbr.createChannelContainer.isEnabled = true

                                }
                            } else {
                                userCheckedItemUidListMbr.removeAt(
                                    userCheckedItemUidListMbr.indexOf(
                                        copyEntity.itemUid
                                    )
                                )
                                ...
                                if (userCheckedItemUidListMbr.size == 0) {
                                    ...
                                    parentViewMbr.bindingMbr.createChannelContainer.isEnabled =
                                        false
            ...
            class ItemLoader {
                data class ViewHolder(
                    val view: View,
                    val binding: ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItemLoaderBinding =
                        ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItemLoaderBinding.bind(
                            view
                        )
                ) : RecyclerView.ViewHolder(view)
            ...
            class Item1 {
                data class ViewHolder(
                    val view: View,
                    val binding: ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItem1Binding =
                        ItemActivityFirebaseChatSampleAddChannelAdapterRecyclerViewItem1Binding.bind(
                            view
                        )
                ) : RecyclerView.ViewHolder(view)
