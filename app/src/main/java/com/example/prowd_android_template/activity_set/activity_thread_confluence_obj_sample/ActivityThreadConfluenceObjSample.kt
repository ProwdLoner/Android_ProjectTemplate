package com.example.prowd_android_template.activity_set.activity_thread_confluence_obj_sample
...
import com.example.prowd_android_template.databinding.ActivityThreadConfluenceObjSampleBinding
...
class ActivityThreadConfluenceObjSample : AppCompatActivity() {
    // <설정 변수 공간>
    // (앱 진입 필수 권한 배열)
    // : 앱 진입에 필요한 권한 배열.
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityThreadConfluenceObjSampleBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityThreadConfluenceObjSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityThreadConfluenceObjSampleSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityThreadConfluenceObjSampleBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityThreadConfluenceObjSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityThreadConfluenceObjSampleSpw(application)
    ...
    private fun onCreateInitView() {
        bindingMbr.threadStartBtn.setOnClickListener {
            bindingMbr.threadStartBtn.isEnabled = false
            bindingMbr.confluenceResult.text = "실행중"

            // 스레드가 모두 완료되면 onComplete 를 실행시킴
            val threadConfluenceObj =
                ThreadConfluenceObj(
                    ...
                    onComplete = {
                        ...
                        runOnUiThread {
                            bindingMbr.threadStartBtn.isEnabled = true
                            bindingMbr.confluenceResult.text = "합류"
                        }
                    }
                )

            // 스레드 1 작업
            val thread1Sec = (1..5).random()
            executorServiceMbr.execute {
                for (millis in 0..thread1Sec * 1000 step 100) {
                    runOnUiThread {
                        bindingMbr.thread1Count.text =
                            ((thread1Sec * 1000f - millis) / 1000f).toString()
                    }
                    Thread.sleep(100)
                }

                runOnUiThread {
                    bindingMbr.thread1Count.text = "0"
                }
            ...
            // 스레드 2 작업
            val thread2Sec = (1..5).random()
            executorServiceMbr.execute {
                for (millis in 0..thread2Sec * 1000 step 100) {
                    runOnUiThread {
                        bindingMbr.thread2Count.text =
                            ((thread2Sec * 1000f - millis) / 1000f).toString()
                    }
                    Thread.sleep(100)
                }

                runOnUiThread {
                    bindingMbr.thread2Count.text = "0"
                }
            ...
            // 스레드 3 작업
            val thread3Sec = (1..5).random()
            executorServiceMbr.execute {
                for (millis in 0..thread3Sec * 1000 step 100) {
                    runOnUiThread {
                        bindingMbr.thread3Count.text =
                            ((thread3Sec * 1000f - millis) / 1000f).toString()
                    }
                    Thread.sleep(100)
                }

                runOnUiThread {
                    bindingMbr.thread3Count.text = "0"
                }
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityThreadConfluenceObjSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityThreadConfluenceObjSampleSpw",
...
//            }
    ...
    class ActivityThreadConfluenceObjSampleAdapterSet {
        // 어뎁터 #1
