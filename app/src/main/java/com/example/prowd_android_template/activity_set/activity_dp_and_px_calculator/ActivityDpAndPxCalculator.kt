...
class ActivityDpAndPxCalculator : AppCompatActivity() {
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityDpAndPxCalculatorBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityDpAndPxCalculatorAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityDpAndPxCalculatorSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityDpAndPxCalculatorBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityDpAndPxCalculatorAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityDpAndPxCalculatorSpw(application)
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityDpAndPxCalculatorSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityDpAndPxCalculatorSpw",
...
//            }
    ...
    class ActivityDpAndPxCalculatorAdapterSet {
        // 어뎁터 #1
