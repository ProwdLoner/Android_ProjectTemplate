package com.example.prowd_android_template.activity_set.activity_web_view_sample
...
import com.example.prowd_android_template.databinding.ActivityWebViewSampleBinding
...
class ActivityWebViewSample : AppCompatActivity() {
    // <설정 변수 공간>
    // (앱 진입 필수 권한 배열)
    // : 앱 진입에 필요한 권한 배열.
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityWebViewSampleBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityWebViewSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityWebViewSampleSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityWebViewSampleBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityWebViewSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityWebViewSampleSpw(application)
    ...
    private fun onCreateInitView() {
        bindingMbr.wView.webViewClient = object : WebViewClient() {
            override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
                super.onPageStarted(view, url, favicon)
                bindingMbr.pBar.visibility = View.VISIBLE
            }

            override fun onPageFinished(view: WebView?, url: String?) {
                super.onPageFinished(view, url)
                bindingMbr.pBar.visibility = View.GONE
            }
        }

        // 웹뷰 설정
        bindingMbr.wView.isHorizontalScrollBarEnabled = false //가로 스크롤
        bindingMbr.wView.isVerticalScrollBarEnabled = false //세로 스크롤
        bindingMbr.wView.scrollBarStyle = WebView.SCROLLBARS_OUTSIDE_OVERLAY // 스크롤 노출 타입
        bindingMbr.wView.isScrollbarFadingEnabled = true // 스크롤 페이딩 처리 여부

        //zoom 허용
        bindingMbr.wView.settings.builtInZoomControls = true
        bindingMbr.wView.settings.setSupportZoom(true)
        bindingMbr.wView.settings.displayZoomControls = false

        bindingMbr.wView.settings.javaScriptCanOpenWindowsAutomatically =
            true //javascript 의 window.open 허용

        bindingMbr.wView.settings.javaScriptEnabled = true //javascript 허용


        //meta태그의 viewport사용 가능
        bindingMbr.wView.settings.useWideViewPort = true
        bindingMbr.wView.settings.loadWithOverviewMode = true

        bindingMbr.wView.settings.layoutAlgorithm = WebSettings.LayoutAlgorithm.TEXT_AUTOSIZING

        bindingMbr.wView.post {
            bindingMbr.wView.loadUrl("https://www.google.com/")
        }
    }
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityWebViewSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityWebViewSampleSpw",
...
//            }
    ...
    class ActivityWebViewSampleAdapterSet {
        // 어뎁터 #1
