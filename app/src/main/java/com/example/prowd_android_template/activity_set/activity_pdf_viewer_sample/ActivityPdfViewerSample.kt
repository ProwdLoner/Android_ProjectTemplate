package com.example.prowd_android_template.activity_set.activity_pdf_viewer_sample
...
import com.example.prowd_android_template.databinding.ActivityPdfViewerSampleBinding
...
class ActivityPdfViewerSample : AppCompatActivity() {
    // <설정 변수 공간>
    // (앱 진입 필수 권한 배열)
    // : 앱 진입에 필요한 권한 배열.
    ...
    // <멤버 변수 공간>
    // (뷰 바인더 객체)
    lateinit var bindingMbr: ActivityPdfViewerSampleBinding

    // (repository 모델)
    lateinit var repositorySetMbr: RepositorySet

    // (어뎁터 객체)
    lateinit var adapterSetMbr: ActivityPdfViewerSampleAdapterSet

    // (SharedPreference 객체)
    // 클래스 비휘발성 저장객체
    lateinit var classSpwMbr: ActivityPdfViewerSampleSpw
    ...
    private fun onCreateInitObject() {
        // 뷰 객체
        bindingMbr = ActivityPdfViewerSampleBinding.inflate(layoutInflater)
        // 뷰 객체 바인딩
        ...
        adapterSetMbr = ActivityPdfViewerSampleAdapterSet()

        // SPW 객체 생성
        classSpwMbr = ActivityPdfViewerSampleSpw(application)
    ...
    // <중첩 클래스 공간>
    // (클래스 비휘발 저장 객체)
    class ActivityPdfViewerSampleSpw(application: Application) {
        // <멤버 변수 공간>
        // SharedPreference 접근 객체
        private val spMbr = application.getSharedPreferences(
            "ActivityPdfViewerSampleSpw",
...
//            }
    ...
    class ActivityPdfViewerSampleAdapterSet {
        // 어뎁터 #1
